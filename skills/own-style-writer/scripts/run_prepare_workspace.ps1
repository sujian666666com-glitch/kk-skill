param(
    ...
    [string[]]$ContentFile = @(),
    [string]$OutputDir = "",
    [ValidateSet("auto", "mineru-precise", "mineru-agent", "markitdown")]
...
$ScriptPath = Join-Path $PSScriptRoot "prepare_writing_workspace.py"
...
$argsList = @("--style-dir", $StyleDir, "--converter", $Converter)
if ($ContentDir) { $argsList += @("--content-dir", $ContentDir) }
foreach ($file in $ContentFile) { $argsList += @("--content-file", $file) }
if ($OutputDir) { $argsList += @("--output-dir", $OutputDir) }
if ($AllowUpload) { $argsList += "--allow-upload" }
...
if ($MineruModel) { $argsList += @("--mineru-model", $MineruModel) }
if ($MaxWaitMinutes -ne 30) { $argsList += @("--max-wait-minutes", [string]$MaxWaitMinutes) }
if ($PollIntervalSeconds -ne 10) { $argsList += @("--poll-interval-seconds", [string]$PollIntervalSeconds) }
